...
- [x] **T12.5 256 MiB の cgroup でバラストが「何を買っているか」と「落ちないか」を測り、要らなければ段階化する**
  ...
  - 結果 (2026-09-10、`18878ad` / `9609b81`): **測った結果、バラストは何も買っていなかったので段階化した** (Opus 案)。
    ...
    - 再デプロイ後は §2 の「デプロイ先の現在地」の RSS 215.9 → 23 MB 前後、`disk_used_percent` 91.7% → 数 % になるはず
...
`0.1.0+d75f1f2` を再デプロイした。**58.6 時間の運用で完了の定義を満たした** (§2 の「デプロイ先の現在地 (2026-09-12)」):
...
**いま手元にある数字** (2026-09-12 の実測 = デプロイ後 58.6 時間。**T13.0 で埋めた**。§2 の「デプロイ先の現在地 (2026-09-12)」が出どころ):
...
- [x] **T13.0 T12.4 の図を 24 時間ぶん読んで、CONNECT 確立の内訳を書く**
  ...
  - 結果 (2026-09-13): デプロイ後 58.6 時間の `/status` `/history` `/metrics` を読んで上の表と判断を書いた
    (§2 の「デプロイ先の現在地 (2026-09-12)」に前後の表)。**採るのは 1 (名前解決の先回りと負のキャッシュ)、2 (接続の山)、
...
**Phase 13 の状態 (2026-09-16、デプロイして 72.7 時間で判定)**: 完了の定義を 1 行ずつ見た (§2 の「デプロイ先の現在地 (2026-09-16)」)。
...
**順番 (2026-09-17 の現状)**: **済** T14.0〜T14.58 (59 / 60。2026-09-16〜17 に連番順・1 タスク 1 エージェントで)。**走行中** 無し。**未着手** push (人) → 再デプロイ (人) → 24 時間 → **T14.99** (Phase 15 の T15.0 は再デプロイ後、T14.99 のデータ取りと並行)。元の順番は「T14.1 → T14.2 → … → T14.58 → push → 再デプロイ → 24 時間 → T14.99」で、そのとおりに進んだ。
...
**終了の記録 (2026-09-17。Phase 14 の実装は全部 main にある。次は人の push と再デプロイ)**:
...
- [x] **T14.0 Phase 13 をデプロイして 24 時間の図と個票を読み、「締めるか、まだ削るか」を決める**
  ...
  - 結果 (2026-09-16): デプロイ後 72.7 時間 (3,930 要求、バーストなし) を読んだ。**判定は Phase 13 の「状態」の表、前後の数字は §2 の
