...
T15.5 の手元での再現も T15.0 と並行してよい。T15.10 と T15.11 は済み。番号の対応: 旧 T15.0 → **T15.12**、
...
- [ ] **T15.5 CPU 割り当て (0.5 コア) を食い切っているものを特定して止める**
  ...
  - **因果はまだ測れていない**: 絞りは `cgroup_cpu` の通算から逆算するとトンネル 2 本が開く **0.7〜2.2 時間前**から始まっており、
    ...
    **重い**。再起動前の別プロセス・別版にも同じ尾がある (7,523 本、p90 58.8 / p95 83.7 ms)。スレッドと接続を結ぶ ID が出力に無いので、
    いま言えるのは相関まで。
  - やること: (1) **まず測る** は **T15.0 (4)(5)(6) に移した** (`/connections` の `tid` / `half_closed` / `spins` / `revents` /
    `idle_secs`、`/profile` の `threads_top`、異常 `cpu_throttled` / `tunnel_spin`)。手元での再現 (片側が先に閉じて残りが黙る
    トンネル) は T15.0 と並行してよい。(2) 直し方の候補は (a) 同じ向きで `splice` が `WouldBlock` を N 回続けたら EOF とみなす、(b) 半閉じのトンネルも
  ...
  - 受け入れ基準: T15.0 (4) の欄で「どのトンネルが回っているか」を確かめてから直すこと (手元の再現か、デプロイ先の再発)。直したあと `/profile` の
