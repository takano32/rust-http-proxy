...
## 2.5 「前提を疑う」ラウンド (TASKS 完了後)

Phase 0〜4 の後、設計上の前提を 8 つの観点から疑い直し、実測で裏付けの取れたものだけ入れた。
**計測の作法**: この機械は big.LITTLE (cpu0-3 = Cortex-A55 / cpu4-7 = Cortex-A78、
システムコールのコストが 2.2 倍違う)。**必ず `taskset` でプロキシを 4-7、ベンチを 0-3 に固定する**。
主指標は CPU/要求 (`/proc/<pid>/stat` の utime+stime ÷ 要求数) と、負荷に依らない
「1 要求あたりの確保回数」「システムコール回数」。スループットは律速がベンチ側に移るので補助。

### 見つかった不具合

- **要求スマグリング** (`6edc748`): `Connection: Content-Length` と指名されると、転送する
  ヘッダーからは落ちるのに本文は送られ、オリジンがそれを次の要求の先頭として読む。
  認証なしの開放プロキシなので誰でも踏めた。枠組みのヘッダーは指名されても落とさないようにした。
...
- **`Pool::sweep()` に呼び出し元が無い** (`0980886`): 負荷が止まってもアイドル接続を解放しない
  (実測: 100 秒放置後も fd 54 のまま → 修正後は 7)。

### 入れた性能改善 (すべて実測付き)

| 変更 | 効果 |
|---|---|
| 素通しできる本文を `splice(2)` で運ぶ (`5eac139`) | 本文 1 MiB の CPU/要求 771.6 → 302.9 us (-60%) |
| オリジンプールの既定を 8 → 64 本、全体上限 256 (`0980886`) | conc=64 の p99 37.3 → 12.1 ms |
| 中継バッファを 64 → 32 KiB (`093c68e`) | 本文 64 KiB で -8%、1 MiB で -11%、1 KiB は退行なし |
| ヘッダー組み立ての `Vec<String>` 撤廃 (`9f552fc`) | 確保 76.1 → 41.1 回/要求 (-46%) |
| キャッシュ HIT の本文直接書き・ヘッダー再解析の省略 (`cbb2c00`) | 256 KiB の HIT で user CPU 50.3 → 24.3 us |

### 計測の結果 **やらなかった** もの

- **統計 `Mutex` の分割**: 統計を丸ごと消したビルドでも `futex` 0.010 vs 0.011 回/要求、
  p99・CPU とも差が出ない。期待効果 0〜0.25%。conc=64 の p99 の真犯人はプールの方だった。
- **`recvfrom` を 3 → 2 回に減らす** (プールの生存確認 `MSG_PEEK` の省略): プールは
  クライアント間で共有されているので、desync すると**他人の応答**が渡り、しかも誤った URL の
  キャッシュとして固定化する。再試行はエラー時しか発火せず代わりにならない。期待値 1% のために
  踏む橋ではない。
- **クライアント書き込みバッファのプール化**: 要求ごとの 64 KiB 確保が malloc を断片化させて
  いるという仮説を実装して測ったが、アイドル接続あたり 30.3 → 30.9 kB、CPU 43.3 → 43.9 us で
  改善なし。アイドル接続の資源はスレッドスタック (実触 16 kB + カーネル 16 kB) が主因。
- **`opt-level = 3`** / **`SO_REUSEPORT`** / **musl 静的リンク**: 上の Phase 3〜4 のとおり。

## 2.6 アイドル接続をスレッドから外す (方針 C)

「1 接続 = 1 スレッドが専任」という前提そのものを外した。要求を処理していない keep-alive
接続はスレッドを手放し、1 本の監視スレッド (epoll) に預ける。5 段階に割って、各段階で
挙動が変わらないことを確かめてから次へ進めた。

| 段階 | 内容 | コミット |
|---|---|---|
| 1 | クライアントの読み取りバッファとストリームを分ける (`ClientBuf` / `ClientReader`) | `c688372` |
| 2 | 接続の状態を `struct Conn` + `enum Step` + `serve_one` / `pump` に組み替える | `931a276` |
| 3 | `sys` に epoll の束縛を足す | `19b4c3d` |
| 4 | `src/idle.rs` `IdleWatch` を足して配線する (既定 off) | `8a4fbc4` |
| 5 | 猶予を読み取りタイムアウトに畳む / 既定 on | `ba8a240`, 本コミット |

### 効果

暇な keep-alive 接続を握ったまま放置 (同じバイナリで `PROXY_PARK_IDLE` を切り替え):

| 接続数 | off | on |
|---|---|---|
| 1,000 | 1,003 スレッド / RSS 44.4 MB | 12 スレッド / 25.6 MB |
| 2,000 | 2,003 スレッド / RSS 72.3 MB | 18 スレッド / 25.7 MB |

1 接続あたり 28.9 kB → ほぼ 0 (接続を増やしても増えない)。
忙しいときの費用は無し (conc=4 の forward、10 秒 × 6 本):
システムコール 5.795 → 5.780 回/要求、CPU 38.35 → 38.69 us/要求 (ぶれ ±0.8% の中)。

### 途中で見つけた不具合

- **要求を処理したスレッドが終わるとプロセスごと abort する** (`f683da4`): 段階 2 で
  スレッドローカルに `Drop` を持つ型を置いてしまい、スレッド終了時の破棄でその置き場自身を
  触って `thread local panicked on drop` になる。ワーカースレッドは 30 秒で自分から終わるので
  **1 要求通してから 30 秒後に必ず落ちていた**。スレッドローカルには `Drop` を持たない中身
  だけを置く形に変え、`try_with` も併用した。回帰テストつき。
- **再検証後の HIT 判定が負荷次第で落ちる** (`69b384f`): テスト側の競合。304 が `max-age=1` を
  返していたので、再検証の完了を待つ間にまた期限が切れていた。

### 設計上の決め事 (レビューで潰した危険)

- `park()` は `Mutex` を `epoll_ctl(ADD)` まで握る (監視スレッドが `conns` にまだ無い記述子の
  事象を見て取りこぼさないように)。生存フラグもロックの中に置く (外の `AtomicBool` だと
  「生きている」と読んだ直後に監視スレッドが終わり、預けた接続が誰にも見られない隙間ができる)。
- 期限は `conns` と `BTreeSet` の両方から必ず同時に消す (片方だけ残ると際限なく育つ)。
- 期限切れの一括 close は 1 周 16 本で切る (`close(2)` は 1 本 10.8us/A78・64.0us/A55。
  上限なしだと 4,096 本で 42/262 ms のあいだ `epoll_wait` に戻れない)。
- 監視スレッドが死んだら預かりぶんを全部閉じて以後の `park` を断る。断られた接続は
  `conn.park = None` にして、以後この接続はスレッドで待つ側に固定する (空回り防止)。
- `impl Drop for Conn` は書かない (`Step::Connect` の部分ムーブが壊れる)。

### 計測の結果 **やらなかった** もの

- **猶予を `poll(2)` で待つ**: 忙しい接続に要求ごとの `ppoll` が 1 回増え (5.78 → 6.81 回)、
  CPU +5.5%。読み取りタイムアウトそのものを猶予の長さにすれば 0 回で済む (`ba8a240`)。
- **`PROXY_PARK_MAX_GRACE`**: 猶予で待つスレッドは「普通に次の要求を読んでいるスレッド」に
  なったので、数えて止める意味が消えた。上と同じコミットで削除。
- **期限切れの close をワーカーに投げる**: 受け渡し (Mutex + channel + スレッド起床) が
  隠したい `close` と同じ桁で、しかも空きスレッド上限 64 を越えると新規スレッドを起こす。
