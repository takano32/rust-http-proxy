...
| ビルドが通る最小のメモリ | 347 MB でも通らない | **99 MB** (手元、T10.11 で再確認。98 MB は落ちる。決めているのは `proxy-http` 85.5 MB / `proxy-blocklist` 84.0 MB — T10.9) — 上限 200 MB に対し **101 MB の余裕**。CI では未測定 | fat LTO なので 350 MB 要る (T9.1、配布は潤沢な環境で作る) |
...
CPU/要求 は 5 秒の計測で ±8% ぶれる。ビルドの通る最小メモリは境目が 1 MB 刻みで薄いので、機械を占有して測る (T10.9)。CI との差は分かっていない。
