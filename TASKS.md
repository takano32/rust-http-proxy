...
番号は付けた順で、並び替えても振り直さない (コミットメッセージから引けるように)。**各 Phase の `Tn.9` は「デプロイ後の様子見と修正、
締めの文書」に予約する** (Phase 14 から。デプロイして数字を見てから書くタスクを、その Phase の最後に置くため)。
...
4. **締める → 採る (T14.9)**。T14.1 と T14.3 を再デプロイしたあとで README と §2 をデプロイ先の数字で書き直し、§0 のゴールに「デプロイ先でも到達 (日付と数字)」を書く。
...
- [x] **T14.0 Phase 13 をデプロイして 24 時間の図と個票を読み、「締めるか、まだ削るか」を決める**
  ...
  - 結果 (2026-09-16): デプロイ後 72.7 時間 (3,930 要求、バーストなし) を読んだ。**判定は Phase 13 の「状態」の表、前後の数字は §2 の
    ...
    初回からこの理由を示した。**採るのは T14.1 (keep-warm)、T14.2 (小物)、T14.9 (デプロイ後の様子見と締め)**。バーストは判定できず (次に来たら表を埋める)。
