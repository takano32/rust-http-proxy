...
**順番 (2026-09-16 に入れ替えた)**: **T14.1 → T14.2 → T14.4 → 再デプロイ → 24 時間 → T14.3**。T14.3 (締めの文書) は
「デプロイ先の数字で書く」タスクなので、T14.4 の `/profile` まで入った版でデータを取ってから書く方が、1 回の再デプロイで済み、
締めの表に段階の内訳 (どこで待っているか) まで載せられる。T14.1 と T14.2 は並列、T14.4 はその 2 つをマージしてから。
...
4. **締める → 採る (T14.3)**。T14.1 のあとで README と §2 をデプロイ先の数字で書き直し、§0 のゴールに「デプロイ先でも到達 (日付と数字)」を書く。
...
- [x] **T14.0 Phase 13 をデプロイして 24 時間の図と個票を読み、「締めるか、まだ削るか」を決める**
  ...
  - 結果 (2026-09-16): デプロイ後 72.7 時間 (3,930 要求、バーストなし) を読んだ。**判定は Phase 13 の「状態」の表、前後の数字は §2 の
    ...
    初回からこの理由を示した。**採るのは T14.1 (keep-warm)、T14.2 (小物)、T14.3 (締め)**。バーストは判定できず (次に来たら表を埋める)。
...
- [ ] **T14.1 名前解決の「熱さ」を TTL から切り離す (keep-warm)**
...
- [ ] **T14.3 締める (README と §2 と §0 をデプロイ先の数字で書き直す)**
  ...
  - やること: **T14.1 と T14.4 を含む版を再デプロイして 24 時間の数字が出てから** (順番は上のとおり。T14.4 のあと)。README の性能節に
...
- [ ] **T14.4 プロファイル画面 (`/profile`): 待ちの段階、スレッドの CPU と状態、ロックの取り合いを 1 枚で見てボトルネックを推定する**
  ...
  - 注意: **測る側が熱い経路を重くしたら本末転倒** (T12.4 と同じ)。時計は境目にしか置かない。`/proc/self/task/*/syscall` は
    ...
    **T14.1 / T14.2 のあとに着手** (`src/lib.rs` `http/mod.rs` `metrics.rs` を触るため)。**T14.3 より先** (再デプロイは T14.4 まで入れて 1 回)。
