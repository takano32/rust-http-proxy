...
| §3 | やったこと (Phase 0〜7) |
| §4 | 測って採らなかったもの |
| §5 | これから (Phase 8〜14。Phase 12 は完了、Phase 13 は実装済みでデプロイ先の確認待ち、Phase 14 は書き出しだけ) |
| 付録 A | 計測の記録 (時系列) |
...
**Phase 13 の状態 (2026-09-13)**: T13.0〜T13.4 のコードはすべて main に入った。マージ後の main で fmt / clippy / **テスト 350 本** /
release ビルド (1,513,088 B、版 `0.1.0+b1756bc`) と **200 MB の cgroup でのクリーンビルド** (42 秒) を確認した。**残っているのはデプロイ先での確認だけ**: 再デプロイして 24 時間後に
`scripts/status-diff.py` (2 枚の差分、`--aaaa`) と `/dashboard`、新しい個票 (`/errors` `/connections` `/dns` `/log` `/hosts`) で
完了の定義 (下) を見る。再デプロイ時に `.rrd` の形式は変わらない (版 2 のまま。統計は消えない)。
...
### Phase 14 — 締めるか、まだ削るか (書き出し。Phase 13 をデプロイして 24 時間の図を見てから決める)

**この節は書き出し** (2026-09-13)。Phase 13 (`0.1.0+b1756bc` 以降) を再デプロイして 24 時間の図と個票が溜まってから、Phase 12〜13 と
同じ手順 (T14.0 で読む → 数字で書ける案だけ残す → Fable と合議) で書き直す。**ここで決めているのは「問い」と「決め方」だけ**で、
タスクは T14.0 しか確定していない。

**Phase 14 の問い**: 名前解決のミスとバースト時の 503 を Phase 13 で片づけたあと、**デプロイ先で利用者が待つものはまだ残っているか。
残っていないなら、このリポジトリは「締める」段階に入る** (§0 のゴール「同じ条件でこれ以上速くならないところまで」は、loopback では
Phase 11 で、デプロイ先では Phase 13 で到達している可能性がある)。

**待つ間に決めてよいこと (数字に依らない)**: どれも小さく、Phase 14 の中身が「締める」でも「削る」でも要る。
...
| README の性能節が loopback の表だけ | §2 | 「デプロイ先の現在地」(2026-09-12 と Phase 13 後) を README にも写す |

**候補と決め方** (T14.0 で 24 時間の図と個票を読んでから。閾値は Phase 13 と同じ考え方):

1. **名前解決**: T13.1 の効きを `dns.misses ÷ 要求` (0.42 → 0.15 未満が基準) と `/dns` の表で確かめる。届いていなければ **理由を個票で
   特定してから** 直す (熱い名前の定義 (直近 TTL 内) が狭いのか、失敗する名前が毎回違うのか、バースト時のリゾルバ混雑なのか)。
   届いていれば名前解決は終わり。
2. **バースト**: `evicted_idle` と `rejected_overload` と `/connections` の占有の内訳。503 が 0 で `/status` が取れていれば終わり。
   `evicted_idle` が毎回のバーストで数十を超えるなら、**上限そのものを見直す** (fd の勘定 4 本/接続 → 実測 2.3 本。`auto` の式を
   3 本にすれば `ulimit -n` 1024 で 320)。
3. **残る待ちの正体**: 平常時の CONNECT 確立 p50 (7.5 ms 以下が基準) と、`/errors` の原因の上位。上位が `reset` / `refused` / `timeout`
   ならオリジン側 (何もしない)。`timeout` が 30 秒で目立つなら `PROXY_TIMEOUT_SECS` の既定 (30) を短くする案 (挙動が変わるので
   数字を見てから)。
4. **締める**: 1〜3 がすべて「終わり」なら、Phase 14 は **§2 と README をデプロイ先の数字で書き直し、§0 のゴールを「デプロイ先でも到達」と
   書く**ことと上の小物で構成する。速さのタスクは入れない。

**Phase 14 に入れないもの**: loopback の CPU/要求 (Phase 12 で決着)、RTT (物理)、TLS (素通し)、自前の DNS クライアント (T13.1 で
足りていれば不要。足りていなくても先に理由を個票で見る)、認証。

- [ ] **T14.0 Phase 13 をデプロイして 24 時間の図と個票を読み、「締めるか、まだ削るか」を決める**
  ...
  - やること: 再デプロイ後 24 時間で `/status` を 2 枚 (差分)、`/history?res=60|3600`、`/metrics`、`/errors?n=500`、`/connections`
    ...
    Phase 13 の完了の定義 (`dns.misses ÷ 要求` 0.15 未満、ミス 1 回 15 ms 未満、平常時の p50 7.5 ms 以下、`rejected_overload` 0 で `/status` が
    取れる、エラーの上位が `dns` でない、個票で「誰が・いつ・なぜ」が読める) を 1 行ずつ判定し、Phase 13 の「状態」を「完了」に書き換える
    (届かなかった行はその理由を個票から書く)。候補 1〜4 のそれぞれに「採る / 採らない」と根拠の数字を書き、採るものを §5 の形式で
    T14.1〜 に起こして Fable と合議する。
  - 受け入れ基準: Phase 13 の完了の定義の各行に 24 時間の実測値と判定が入っていること。候補 1〜4 のすべてに数字つきの判断が
    書いてあること。Phase 14 のタスクがすべて「受け入れ基準を数字で書けるか」で選別されていること。

**Phase 14 の完了の定義 (仮)**: 「締める」なら、README と §2 がデプロイ先の数字で書かれ、§0 のゴールに「デプロイ先でも到達 (日付と数字)」が
書いてあり、上の小物が全部済んでいること。「まだ削る」なら T14.0 で置き直す。
