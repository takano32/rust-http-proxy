...
**Phase 12 の状態 (2026-09-10)**: T12.0〜T12.7 のコードはすべて main に入った (T12.2 は測って採らない)。手元の受け入れ基準は
すべて満たし、マージ後の main で fmt / clippy / テスト 310 本 / release ビルド / **200 MB の cgroup でのクリーンビルド** (46 秒) を確認した。
**残っているのはデプロイ先での確認だけ**: 再デプロイして 24 時間後に T12.0 の差分 (`scripts/status-diff.py`) と `/dashboard` の
「CONNECT 確立 p50」で、AAAA ありのホストが 10 ms 台になっていること・`GET /` が 200 なこと・RSS が 23 MB 前後になっていることを見る。
再デプロイで `.rrd` は版 2 に作り直されるので、通算の統計は 1 度消える。`.env` に `PROXY_IPV6=off` の回避を入れていたなら外してよい
(T12.1 が自動で IPv4 を先頭にする。外せば `/status` の `ipv6` で IPv6 が生き返ったかも見える)。
