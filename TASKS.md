...
**この節の数字は 2026-09-08 に T10.0 が一度に測り直したもの** (HEAD `2a3dca4`、rustc 1.96.0、8 コア / 6.6 GiB、loopback)。
道具は `scripts/cpu-per-request.sh` (**プロキシを cpu4-7・ベンチを cpu0-3 に固定**、`--lite`、本文 1 KiB、
10 秒 × 3 回の中央値。暇な接続の 2 行とシステムコールの 3 行は 1 回)。
**配布するバイナリは `dist` (fat LTO) なので両方を載せる**。
「着手前」は 2026-09-07 の Python ベンチ (付録 A)。出典が別のものは行に書いた。

| 項目 | 着手前 | `release` | `dist` (配布用) |
|---|---|---|---|
| forward, 8 並列 (`--lite`) | 188 req/s, p50 42.0 ms | **37,899 req/s, p50 0.178 ms** (202 倍) | **38,757 req/s, p50 0.175 ms** |
| forward, 8 並列 (キャッシュ HIT) | — | 67,269 req/s, p50 0.096 ms | **69,901 req/s, p50 0.092 ms** |
| forward, 64 並列 | 1,517 req/s, p50 41.9 ms | 30,916 req/s, p50 1.79 ms, p99 7.9 ms | 31,076 req/s, p50 1.78 ms |
| 1 接続 1 要求 (`--no-keepalive`) | — | 14,167 req/s, p50 0.308 ms | 14,051 req/s, p50 0.308 ms |
| CONNECT 確立, 8 並列 | 3,506 tunnels/s | 8,883 /s, p50 0.187 ms | 9,549 /s, p50 0.191 ms |
| CONNECT トンネル 1 本 (**ベンチ律速**) | 1,044 MiB/s | **3,986 MiB/s** (プロキシは 1 コアの 73%) | 3,945 MiB/s (同 72%) |
| CPU/要求 (forward / HIT / 64 並列) | — | **42.7 / 37.3 / 62.0 us** | **40.6 / 34.5 / 60.5 us** |
| CPU (1 接続 1 要求 / CONNECT 確立 / トンネル) | — | 102.7 us/接続 / 165.3 us/本 / 351.6 us/MiB | 101.5 / 168.1 / 351.6 |
| トンネル 1 本あたりのスレッド | 3 | **1 (中継中) / 0 (暇なとき)** | 同左 |
| 暇な CONNECT トンネル 5,000 本 | 5,005 スレッド / RSS 93.9 MB | **68 スレッド / 72.1 MB** | 68 スレッド / 64.7 MB |
| 暇な keep-alive 接続 2,000 本 | 2,003 スレッド / RSS 72.3 MB | **19 スレッド / 25.9 MB** | 19 スレッド / 26.5 MB |
| 1 要求あたりのシステムコール | 10.06 | **5.02** (`recvfrom` 3 + `sendto` 2) | 5.01 |
| 1 接続あたりのシステムコール (`--no-keepalive`) | 33.16 (T4.4 前) | **11.02** | 11.02 |
| CONNECT 1 本あたりのシステムコール | — | 29.06 | 29.05 |
| 1 要求あたりの確保回数 | 98.7 | **10.0** (T9.5 時点。T9.5 で指標として無効と分かったので測り直していない) | — |
| ビルドが通る最小のメモリ | 347 MB でも通らない | **99〜100 MB** (手元、T10.9。98 MB は落ちる。決めているのは `proxy-http` 85.5 MB / `proxy-blocklist` 84.0 MB) — 上限 200 MB に対し **100 MB の余裕**。CI では未測定 | fat LTO なので 350 MB 要る (T9.1、配布は潤沢な環境で作る) |
| バイナリ | 857 KB | 1,381,872 B | **988,648 B** |
| テスト | 150 単体 + 21 結合 | **200 単体 + 52 結合** (T10.9 時点、全通過) | — |

`dist` は `release` に対して forward +2.3% (CPU -4.9%)、HIT +3.9% (CPU -7.6%)、64 並列 +0.5%、
CONNECT 確立 +7.5%、1 接続 1 要求 -0.8%、トンネル -3.2% (ぶれの中)。**速さでも小ささでも `dist` が上**で、
...
- [x] **トンネル 1 本の CPU が 200 us/MiB を切る** — **182.7 us/MiB** (user 10.9 / kernel 171.8。ほぼ全部 `splice`)。
  同じ条件のスループットは **3,956 MiB/s (3.86 GiB/s)** で旧条件「1.5 GiB/s 以上」も満たすが、
  **この MiB/s はベンチ律速**なので完了条件には使わない (プロキシは 1 コアの 72% しか使わず、飽和するのはベンチの blaster)。
