...
- [ ] **T8.6 ビルドのメモリの余裕を測る**
  - 目的: いま 200 MB の cgroup で通ることは確かめてあるが、**余裕がどれだけあるかは分かっていない**
    (通るか通らないかの 2 値でしか見ていない)。行が増えたときに、いつ落ちるのかが読めない。
  - やること: `scripts/build-memory.sh --find 150 160 170 180 190 200` を回して、通る最小の上限を出す。
    そこから逆算して「あと何行入るか」を TASKS に書く。
  - 受け入れ基準: 通る最小の上限が数字で分かること。
