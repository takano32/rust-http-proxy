...
(2026-09-16 の指示「何回も回してもムダだ」。エージェントは `cargo fmt --all`、触ったクレートの `cargo clippy -p <crate> --all-targets -- -D warnings`、
触ったクレートのテストと自分の結合テスト (`cargo test -p <crate>` / `--test <名前>`) だけを回して手放す。全体の clippy / test / release ビルドは
