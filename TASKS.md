...
- **判定の外で起きたこと (2026-09-18、記録として)**: 雪像の 86 秒前に新しい接続元 (クラウドの IP) が現れ、15 分で 92 件の
  forward を投げた。宛先は**その接続元自身の A レコード**で、応答は 308、73 件はプロキシのキャッシュ HIT — 開放プロキシの
  当たり確認の形。その 23 分後 (05:55〜05:57Z) に**素性の無関係な第三者のホストへ CONNECT が 5 本成立している**
  (4 本はポート 80、1 本は 443 で TLS が 5.7 kB 流れた)。つまり**当たり確認は成功し、第三者中継の段階に入った**。
  いまの設定は断る条件が 1 つも無い (`PROXY_ALLOW_CLIENTS` 空 / `PROXY_MAX_CONNS_PER_CLIENT` 0 / `PROXY_CONNECT_PORTS` 空 /
  ブロックリスト 0 件) ので、「断った 0 件」は「断るべきものが来なかった」ではなく**断る条件が無い**の 0。
  認証を入れないのは §0 の決まり、認証なしで公開するのは利用者の決定なので、ここでは是非を書かず数字だけ残す → **T15.10**。
...
- **判定の外で起きたこと (2026-09-18、記録として)**: 雪像の 86 秒前に新しい接続元 (クラウドの IP) が現れ、15 分で 92 件の forward で当たり確認をし (宛先はその接続元自身の A レコード、応答 308、73 件はキャッシュ HIT)、その 23 分後に**素性の無関係な第三者のホストへ CONNECT を 5 本通した** (4 本はポート 80、1 本は 443 で TLS 5.7 kB)。**開放プロキシとして発見され、当たり確認が成功した**段階。いまは断る条件が 1 つも無い (`PROXY_ALLOW_CLIENTS` 空 / `PROXY_MAX_CONNS_PER_CLIENT` 0 / `PROXY_CONNECT_PORTS` 空 / ブロックリスト 0 件) ので「断った 0 件」は「断る条件が無い」の 0。認証を入れないのは §0 の決まり、認証なしで公開するのは利用者の決定なので**是非は書かず数字だけ残す** → T15.10。
...
- [ ] **T15.10 `PROXY_MAX_CONNS_PER_CLIENT` と `PROXY_CONNECT_PORTS` を既定のままにするかを 1 回だけ決める**
  ...
  - 材料: (1) 上限 240 のうち **123 本 (51%) を 1 つの接続元が占めた**実測 (`/bursts` の写真、`clients_distinct` 1、
    ...
    その 23 分後に**素性の無関係な第三者のホストへ CONNECT を 5 本通した** (4 本はポート 80、1 本は 443 で TLS 5.7 kB)。
