...
本文中の「§2 の『デプロイ先の現在地 (2026-09-12)』」などの参照はそちらを指す。
...
| RSS | 21.1 MB (Phase 13) | **41.8 MB** (十進。雪像を配っている最中に読んだ値は 42.8 MB で、うち約 3.6 MB は**この測定自身**が増やしたぶん) | `$.status.memory.rss` と `/history?res=5` の 5 秒刻み |
...
| エラー | 101 件 (Phase 12 のバースト) / 0 (Phase 13) | **4 件**。すべて 2026-09-16 の 47 分間 (22:49〜23:32、うち 3 件は山と同じ数分) に集中した `161.33.196.121` の CONNECT 502 で、原因は timeout 2 / refused 1 / dns 1。403 は 0 件 | `$.errors.errors` |
...
| `/profile` の段階の内訳 (T14.3) | (無い) | 6.0 時間で **CONNECT 433 本** (avg: queue 4.93 / client_read 1.12 / dns 1.13 / connect 15.98 / first_relay 20.73 / relay 377.6 / park 71,308 ms)、**forward 97 本** (origin 26.4 / ttfb 50.0 ms、うち 76 本はキャッシュ HIT で <1 ms)。`queue` は **二峰性**で 433 本が 1 ms 未満・**46 本 (9.2%) が 25〜76 ms**・100 ms 超は 0 本 | `/profile?res=60` (362 標本。1,440 標本のうち 256 KiB で切れて 6.0 時間ぶんしか返らない) |
...
| バーストがあっても `rejected_overload` 0 で `/status` が取れる | = 0 | 36.9 時間ずっと 0 (山 171 のときも 0) | **満たした** |
...
- **discord.com だけ 0.15**。理由は測れている: **2〜4 本の塊が約 1,800 秒おきに来る**ので `PROXY_DNS_WARM_SECS` の 900 秒の窓を
  いつも外れ、塊の 1 本目が必ずミスし、2 本目が TTL 60 秒より遅れて来るともう 1 回ミスする。1,553 回の引き直しの 58% は
  誰も待っていない 900 秒の尻尾に消えていた。窓を 3,600 秒にした再生ではこの 21 ミスが 1 (再生の頭の準備運動) になる → **T15.4**。
...
- **物差しの但し書き** (次に読む人のために):
  - 「平常時 = 1 標本 300 本未満」の切り方は、同時接続 32 の本当のバースト 1 時間 (ミス率 0.381) を**残し**、同時接続 6 / 8 で
    本数だけ多い 2 時間 (ミス率 0.028 / 0.019) を**捨てる**向きに働く。9 通りの切り方で 0.094〜0.147 のばらつきがあり、
    決まりどおりの **0.144 はまともな読み方の中でいちばん悪い値** (定常状態は 0.09〜0.11)。切らずに全時間で読むと 0.147 で、
    基準 0.15 に対する余裕は 2%。
...
- **締め (2026-09-18、T14.99)**: 再デプロイの 36.6 時間後 (2026-09-18 05:32:13 UTC) に雪像を 1 枚取り、**5 人が独立に読んで互いの主張を反証し合った**うえで §2 / §0 / 判定 / Phase 15 を書いた。**Phase 14 の完了の定義は 4 行中 2 行を満たし、2 行は届かず** (手引き (d) の 16 行は 満たした 10 / 一部 5 / 届かず 1)。届かなかった **discord.com のミス率 0.15** は原因が測れていて Phase 15 に持ち越し (T15.4、窓 900 → 3,600 秒の再生で 21 → 1)。届かなかった **確立 p50 6 ms** は**基準の立て方が誤りだった**ので追わない (中央値は既に 1 RTT。§4 に 1 行)。**分析で見つかった物差しの誤りが 3 つ**: 確立の時計に名前解決が入っていない (T15.1)、確立に `queue` が入っていない (T15.2)、引き直しの判定が通算の速さとその瞬間の件数を比べていて読む分で裏返る (T15.3)。**コードは 1 行も触っていない** (`docs:` のみ)。
...
- [x] **T14.99 締める (README と §2 と §0 をデプロイ先の数字で書き直す)**
  ...
  - 結果 (2026-09-18): 2026-09-18 05:32:13 UTC (起動 1789577651 から **36.6 時間**) に `scripts/collect-deployed.sh` で雪像を 1 枚取り
    ...
    **満たした 10 / 一部 5 / 届かず 1**。判定の中身と「なぜ届かなかったか」は §2 の「デプロイ先の現在地 (2026-09-18)」の
    ...
    次の手は Phase 15 の T15.1〜T15.11**。コードは 1 行も触っていない (`docs:` のみ)。
...
- [ ] **T15.5 CPU 割り当て (0.5 コア) を食い切っているものを特定して止める**
  ...
  - 見積もりと費用: CPU 0.500 コア → 0.002 コア台 (家事のスレッドの水準)。**確立の p50 は動かない見込み** (既に 1 RTT)。
    効きを見るのは `queue` の段で、6.0 時間 500 本のうち **46 本 (9.2%) が 25〜76 ms・100 ms 超は 0 本** = 100 ms の周期そのものの形。
...
- [ ] **T15.6 connect のタイムアウトを 30 → 10 秒にする**
  ...
  - 見積もりと費用: 待ちが 30 → 10 秒。起きた回数は 36.6 時間で 3 回 (3,301 要求の 0.09%)。ただし**山の時間帯の平均 281.9 ms の
...
- [ ] **T15.9 雪像 1 枚で全部読めるようにする (切れている部を隠さない)**
  ...
  - やること: (1) **いちばん安い** — 要約の先頭に「切れている部」の 1 行を出す (全部の部を回って `truncated` が true のものを
    `count` / `shown` つきで並べる)。あわせて個票の節に「そろっている窓 = `min(at + secs)` 〜 `taken_at`」と
    「ミスの数は下限 (`ms.dns` はミリ秒の整数)」を 1 行ずつ。(2) `/recent` に `before=` か `offset=` を足す (2,000 本が 4 回の要求で
...
- [ ] **T15.10 `PROXY_MAX_CONNS_PER_CLIENT` と `PROXY_CONNECT_PORTS` を既定のままにするかを 1 回だけ決める**
  ...
  - いまの設定はどれも既定: `PROXY_ALLOW_CLIENTS` 空 / `PROXY_MAX_CONNS_PER_CLIENT` 0 (無効) / `PROXY_CONNECT_PORTS` 空
    (**ポート制限なし**。実際にポート 80 への CONNECT が 4 本通っている) / ブロックリスト 0 件。だから「断った 0 件」は
    「断るべきものが来なかった」ではなく**断る条件が 1 つも無い**の 0。
...
- [ ] **T15.11 1 枚の要約の直し (`snapshot-summary.py` / `snapshot-diff.py` の 5 件)**
...
T14.99 (2026-09-18) で §2 から移したもの。**1 文字も変えていない** (本文中の「§2 の『デプロイ先の現在地 (2026-09-12)』」などの参照はここを指す)。最新の 1 枚は §2 の「デプロイ先の現在地 (2026-09-18)」。
