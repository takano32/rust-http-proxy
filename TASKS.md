...
| 暇な keep-alive 接続 2,000 本 | 2,003 スレッド / RSS 72.3 MB | **24 スレッド / 20.8 MB** ※ | 25 スレッド / 23.5 MB ※ |
...
※ **この 1 行だけ §1 に手順が無い。** ベンチに「暇な keep-alive 接続を握る」モードが無く、
T10.0 も T10.11 も使い捨てツールで測っている (リポジトリには入っていない)。
再現できるようにするなら `crates/bench` に `--only idle-conns` を足すこと (T11.4)。
...
- [ ] **T11.4 ベンチに `--only idle-conns` を足す (§2 の最後の再現できない行)**
