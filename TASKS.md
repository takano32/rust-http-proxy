...
| forward, 8 並列 (**既定プロファイル**、warn) | — | 31,155 req/s — CPU **51.4 us** (T11.2。`--lite` より +25.2%。差は合流の待ち → T11.9) | — |
...
- [ ] **T11.3 `refresh.rs` の背景再検証を `Workers` の上限の内側に入れる**
...
- [ ] **T11.9 保存されない URL で同時ミスの合流を待たない**
