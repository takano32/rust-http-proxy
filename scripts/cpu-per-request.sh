...
#
# 使い方:
#   scripts/cpu-per-request.sh [--only forward|connect|tunnel] [bench の残りの引数...]
...
awk -v u=$((u1 - u0)) -v s=$((s1 - s0)) -v ops="$ops" -v tick="$tick" -v hwm="$hwm" \
    -v t0="$threads0" -v t1="$threads1" -v unit="$unit" 'BEGIN {
  us = 1e6 / tick
  printf "CPU/%s: %.2f us (user %.2f / kernel %.2f)  %s %d  proxy peak RSS %.1f MB  threads %d -> %d\n",
    unit, (u + s) * us / ops, u * us / ops, s * us / ops, unit, ops, hwm / 1024, t0, t1
}'
