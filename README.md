...
- **`discord.com` の名前解決のミス率 0.15** (基準は 0.05 未満)。`discord.com` は **2〜4 本の塊が約 1,800 秒おき**に
  ...
  デプロイ先での効きの判定は T15.99 です (`TASKS.md`)。
...
**直す前 (2026-09-10) との比較**は日付ごとの表に残してあります (`TASKS.md` の**付録 A「デプロイ先の現在地の
...
入れ替えも生き延びているので、次の Phase は「まず測る」から始めます (`TASKS.md` の T15.0 で計器を先に足し、そのあと T15.5)。
...
責務ごとの層に分けてあります (35 + 本体)。分けている理由は 2 つで、責務を 1 つに保つことと、`rustc` がクレート単位で
...
- **判定表の「平常時」は本数で切っています** (1 標本 300 本未満)。同時接続の山ではなく本数で切るので、
  ...
  **判定がぎりぎりのときは切り方を変えて幅も見てください** (内訳は `TASKS.md` §2 の「物差しの但し書き」)。
...
再デプロイ後 24 時間の数字で `TASKS.md` の §2 と §0 を書き直すのが、その Phase の `Tn.99` (Phase 14 は `T14.99`、いまは `T15.99`) です。
