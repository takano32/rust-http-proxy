...
| `PROXY_PARK_IDLE` | `off` | アイドルな keep-alive 接続をスレッドから外し、1 本の監視スレッド (epoll) に預ける。Linux 専用 (それ以外では自動的に無効)。実測: 暇な接続 1,000 本でスレッド 1,003 → 13 本、RSS 45.8 → 25.3 MB (30.3 → 8.3 kB/接続) |
| `PROXY_PARK_GRACE_MS` | `3` | 預ける前に同じスレッドで待ってみる時間 (ミリ秒)。続けて要求が来る忙しい接続に、預ける/戻すの往復 (`epoll_ctl` 2 回 + ワーカーの受け渡し) を払わせないための猶予。`0` なら猶予なしで即座に預ける |
| `PROXY_PARK_MAX_GRACE` | `256` | 同時に猶予待ちできるスレッド数の上限。全接続がいっせいに暇になったときに、猶予のあいだスレッドが積み上がるのを止める。`0` で無制限 |
