...
| 項目 | 前 (Phase 12〜14) | いま (Phase 15、126.6 時間) |
|---|---|---|
| CONNECT 確立 (平常時) | avg 25.3 / **p50 8.2** / p95 84.1 ms | avg 21.9 / **p50 8.1** / p95 187.0 ms |
| 利用者が待つ `wait` (平常時) | (T15.0 より前は無い) | **p50 約 8.2 / p95 約 189 ms** (次の基準線) |
| 名前解決のミス (平常時) | **0.40 回/接続**、接続 1 本あたり 5.3 ms | **0.05 回/接続**、接続 1 本あたり **1.6 ms** |
| `discord.com` のミス率 | **0.441** (T15.0 の版の 9 時間) | **0.01** (1,600 要求 / 10 ミス) |
| CPU の絞り (0.5 コアの枠) | 毎周期 (Phase 14。0.5 コアに張り付き) | **0 / 889,600 周期**、プロセス全体 0.0014 コア |
| connect の `timeout` | 0.04 件/時 | **0 件** (既定 10 秒のまま。最大 3,197 ms) |
| RSS | 41.8 MB (Phase 14) | **47.0 MB** (= 44.8 MiB) |

**出どころ (雪像の中の場所)** と、表に入りきらなかった続き:

- **CONNECT 確立 / `wait` / 名前解決のミス** — `$.history["3600"].samples` の `connect_buckets` と `wait` を段ごとに足して線形補間、
  ミスは `dns_misses` / `connects` / `dns_ms_sum`。日ごと (`/daily` の `dns_per_connect`) では 0.02〜0.12。
  参考: 直近 1,024 本の `$.status.recent_quantiles.wait` は p90 111 / p99 113 ms (この 1,024 本はバーストの中)
- **`discord.com` のミス率** — 前後 2 枚の `$.hosts.hosts` の差分 (`http-intake.logs.us5.datadoghq.com` 0.00、`mtalk.google.com` 0.01)
- **CPU の絞り** — `$.status.kernel.cgroup_cpu.since_start` と `/profile?res=60`。`/healthz` の `checks.cpu` も ok
- **connect の `timeout`** — `$.history["3600"]` の `errors_by_cause` (後の期間はエラー 0 件)
- **RSS** — `$.status.memory.rss` (47,005,696 B)

**p95 の尾 (約 187〜189 ms) は相手までの距離のせいで、コードでは縮まない**。

- 平常時の p90 は `wait` 約 120 / `connect` 約 117 ms で、差は約 3 ms しかありません (平均の差 1.6 ms は名前解決)。
  尾は遠い相手への connect (100〜250 ms の区間に 10.7%) で、`run_delay_us` (走れずに待った時間) は conn 役で
  5.45 時間に合計 72 ms、CPU の絞りも 0 です。p50 は 8.2 → 8.1 ms で、1 往復 (1 RTT) のままです。

**直す前 (2026-09-10) との比較**は日付ごとの表に残してあります (`TODO.md` の**付録 A「デプロイ先の現在地の
古い版」**。2026-09-10 / 2026-09-12 / 2026-09-16 / 2026-09-18 の 4 枚を 1 文字も変えずに移しました。2026-09-18 の 1 枚には
Phase 14 の名前解決 0.55 → 0.144 回/接続と、p50 6 ms に届かなかった理由があります)。山場は 5 つで、
