...
- **`discord.com` の名前解決のミス率 0.15** (基準は 0.05 未満)。`discord.com` は **2〜4 本の塊が約 1,800 秒おき**に
  来るので、keep-warm の窓 (`PROXY_DNS_WARM_SECS`、既定 900 秒) をいつも外れ、塊の 1 本目が必ずミスします。
  窓を 3,600 秒にして個票を再生するとこのミスはほぼ消えるので、次はそこを 1 行変えて確かめます (`TASKS.md` の T15.4)。
