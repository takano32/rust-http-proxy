...
**いまの数字は 2026-09-18、Phase 14 を再デプロイして 36.6 時間の実測**です。出どころは雪像 1 枚
(`scripts/collect-deployed.sh` が取った `~/rust-http-proxy-status/2026-09-18T053213Z-snapshot.json`)。
前後は `$.history["3600"].samples` を再起動時刻 (epoch 1789577651) で切り、**バーストの無い時間帯
(1 標本 300 本未満) どうし**で並べています (前 = 146 標本 7,523 本、いま = 33 標本 2,034 本)。
`TODO.md` の §2「デプロイ先の現在地 (2026-09-18)」と同じ数字です。**雪像はリポジトリに入れません**
...
- **`discord.com` の名前解決のミス率 0.15** (基準は 0.05 未満)。`discord.com` は **2〜4 本の塊が約 1,800 秒おき**に
  来るので、keep-warm の窓 (`PROXY_DNS_WARM_SECS`、当時の既定 900 秒) をいつも外れ、塊の 1 本目が必ずミスします。
  窓を 3,600 秒にして個票を再生するとこのミスはほぼ消えるので、**T15.4 で既定を 3,600 秒にしました**。
  デプロイ先での効きの判定は T15.99 です (`TODO.md`)。
- **平常時の CONNECT 確立 p50 8.4 ms** (基準は 6 ms 以下)。**これは追いかけても届きません**。確立の中央値は
  すでにオリジンまでの 1 往復そのもので (確立 − カーネルが測った RTT の中央値が **−0.21 ms**)、個票 614 本の
  `rtt_ms.origin` は最小 6.72 ms・p50 8.22 ms、**6 ms 以下の相手が 1 本もありません**。6 ms という基準の方が
  誤っていました (名前解決の**平均** 6.3 ms/接続 を中央値から引ける、と考えた数字でした)。

**直す前 (2026-09-10) との比較**は日付ごとの表に残してあります (`TODO.md` の**付録 A「デプロイ先の現在地の
古い版」**。2026-09-10 / 2026-09-12 / 2026-09-16 の 3 枚を 1 文字も変えずに移しました)。山場は 5 つで、
