...
- **ヘルスチェック & メトリクス & 操作**:
  ...
  - **個票 (集計では読めない「誰が・いつ・なぜ」。T13.4)**: `/errors?n=100` で直近のエラー
    ...
    **`/clients?sort=requests|recent|targets|literal&limit=200`** (最大 1,000) で**全接続元**の個票 (T14.7)。
    ...
    名前を引かずに繋いでいる数)、`nonstandard_ports` (443 / 80 以外への要求数) が出ます。
  ...
  - **再起動をまたぐか (`persisted` / `restored`)**: `/recent` `/errors` `/bursts` `/log` の 4 つは、
    ...
    **再起動前から引き継いだ件数**です。ファイルの中は 4 本の環状の領域で、
    閉じた接続 8,192 件 / エラー 2,048 件 / 山の写真 128 枚 / ログ 4,080 行 (どれもメモリのリングより多く持つので、
...
| `PROXY_STATS_PERSIST` | `on` | 統計と履歴を `$HOME/.rust-http-proxy.rrd` (固定 4 MiB) に、**個票 (`/recent` `/errors` `/bursts` `/log`) を `$HOME/.rust-http-proxy.recent` (固定 4 MiB)** に残し、再起動後に読み戻す。`off` で無効 (どちらのファイルも作らず、履歴の収集スレッドも起動しないので `/history` とダッシュボードのグラフは空になり、個票の `"persisted"` は `false` になる) |
