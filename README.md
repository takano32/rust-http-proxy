...
(ビルドはメモリ 120〜125 MB で通ります。下の「ビルド・テスト」を参照)。
...
> このリポジトリは **120〜125 MB のメモリでリリースビルドが通ります** (2026-09-19、手元の aarch64 で実測。
> 115 MB 以下は測った範囲では毎回落ちます。**120 MB は境目で、同じ機械・同じコミットでも回ごとに通ったり落ちたりします**
> — 同じ日に 2 つのコミットで 6 回ずつ測って 2/6 と 0/6、測り直しでさらに 1/2。125 MB は測った範囲では毎回通ります。
> 落ちるのはいつも `proxy-metrics-window` のところです。CI が毎回確かめているのは **180 MB の関門を通るかどうかだけ**で、
> CI で通る最小は測っていません)。LTO を既定で切っているのと、要求の経路に乗らないクレートを `codegen-units = 4` に
> してあるのもこのためです (下の「プロファイルの設定」)。
> `rustc` はクレート単位で全部を一度に抱えるため、いちばん大きいクレート (`proxy-metrics-window`) の 1 プロセスで
> **RssAnon 107.8 MB** 使い (通る最小はここに 12〜20 MB 足した値になります。`/usr/bin/time -v` の最大 RSS は
...
| ビルドが通る最小のメモリ | **120〜125 MB** (手元。回ごとに揺れる。CI では未測定) | 350 MB 以上 |
| バイナリ | 2,563,896 B | **1,842,976 B** (721 KB 小さい) |
| CPU/要求 (forward 8 並列) | 41.4 us | **39.6 us** |

(バイナリの大きさと通る最小のメモリは 2026-09-19 (T15.12 段 5・段 6') の実測です。2026-09-08 の時点は
`release` 1,381,872 B / `dist` 1,054,184 B (327 KB 小さい) でした。CPU/要求 の 2 つは 2026-09-08 の値で、
段 5 では測り直していません。速さの測り方は上の「性能」と同じです)
