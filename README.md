...
(ビルドはメモリ約 90 MB で通ります。下の「ビルド・テスト」を参照)。
...
比べるときは必ず同じ固定の仕方で測ってください。数字は 2026-09-08 の実測で、`TODO.md` の §2 と同じものです
...
**いまの数字は 2026-09-26、Phase 16 (T16.0 の計器 + `codegen-units = 4`) を再デプロイして 47.2 時間の実測**です。出どころは雪像 1 枚
(`scripts/collect-deployed.sh` が取った `status/2026-09-26T114602Z-snapshot.json` と、同時刻の
`/healthz` `/profile?res=60` `/daily`)。版は `0.1.0+2e57626` です。「いま」は `$.history["3600"].samples` を再起動時刻
(2026-09-24 12:36:19Z) で切り、**バーストの無い時間帯 (1 標本 300 本未満)** だけを足したもの (37 標本 2,477 本)、
「前」は 2026-09-24 の 1 枚 (Phase 15、126.6 時間。平常時 111 標本 5,452 本) の値です。`TODO.md` の §2「デプロイ先の現在地 (2026-09-26)」と同じ数字です。
**雪像はリポジトリに入れません** (利用者の閲覧先ホストと接続元 IP が並びます)。

| 項目 | 前 (Phase 15、126.6 時間) | いま (Phase 16、47.2 時間) |
|---|---|---|
| CONNECT 確立 (平常時) | avg 21.9 / **p50 8.1** / p95 187.0 ms | avg 19.0 / **p50 7.8** / p95 159.9 ms |
| 利用者が待つ `wait` (平常時) | **p50 約 8.2 / p95 約 189 ms** | **p50 7.9 / p95 167.9 ms** (基準線) |
| 名前解決のミス (平常時) | **0.05 回/接続**、ミス 1 回 33.2 ms、接続 1 本あたり 1.6 ms | **0.06 回/接続**、ミス 1 回 **106.8 ms**、接続 1 本あたり **6.1 ms** |
| `discord.com` のミス率 | 0.01 (1,600 要求 / 10 ミス) | **0.02** (355 要求 / 8 ミス) |
| 裏の引き直し | 359 回/時、名前ごとの最大 78.2 回/時 | **436 回/時**、名前ごとの最大 **80.0 回/時** (閾 80 ちょうど) |
| warm の枠 (上限 32) | (欄が無い) | `dns_warm_max` **15**、追い出し (`warm_evicted`) **0** |
| CPU の絞り (0.5 コアの枠) | 0 / 889,600 周期、プロセス全体 0.0014 コア | **0 / 412,474 周期**、プロセス全体 **0.0013 コア** (cgroup の通算 0.0022 コア、ユーザー空間 31%) |
| connect の `timeout` | 0 件 | **0 件** (エラーは 2 件で、どちらも宛先の名前が引けない 502) |
| RSS | 47.0 MB | **54.2 MB** (= 51.7 MiB。増えたのは `heap_free` だけ) |

**出どころ (雪像の中の場所)** と、表に入りきらなかった続き:

- **CONNECT 確立 / `wait` / 名前解決のミス** — `$.history["3600"].samples` の `connect_buckets` と `wait` を段ごとに足して線形補間、
  ミスは `dns_misses` / `connects` / `dns_ms_sum`。この版の日ごと (`/daily` の `dns_per_connect`) は 2 日ともバーストの日で 0.001。
  参考: 直近 1,024 本 (14.3 時間ぶん) の `$.status.recent_quantiles.wait` は p50 7.8 / p90 16.0 / p99 269 ms
- **`discord.com` のミス率** — 前後 2 枚の `$.hosts.hosts` の差分 (`http-intake.logs.us5.datadoghq.com` 0.00、`mtalk.google.com` はこの間に要求 0)
- **裏の引き直し / warm の枠** — `$.status.dns.refreshes` ÷ `uptime_secs` と `$.dns.entries`、`$.history["3600"]` の `dns_warm_max` (T16.0)、`$.status.dns.warm_evicted`
- **CPU の絞り** — `$.status.kernel.cgroup_cpu.since_start` (`usage_usec` / `user_usec` は T16.0 から) と `/profile?res=60`。`/healthz` の `checks.cpu` も ok
- **connect の `timeout`** — `$.history["3600"]` の `errors_by_cause` と `$.errors.errors`
- **RSS** — `$.status.memory.rss` (54,231,040 B)。`heap_free` 11.9 → 21.2 MB (解放したあとも持っているヒープ) の増えで、
  使っているヒープ・`mmap`・記録のリングはほぼ同じ = リークではありません
- **判定表** — `scripts/snapshot-diff.py … --criteria phase17` (上) は満たした 5 行・届かず 2 行・判定できず 1 行。届かずは
  conn 役の CPU/要求 (1.41 倍。材料が 5 時間ぶん・要求 366 本しかなく、MiB あたりでは向きが逆) と `/events` の `dns_slow` 0.61 件/時 (閾 0.1)

**名前解決はミスの率が変わらず、1 回の重さが上がりました** (33.2 → 106.8 ms)。よく使う相手のミスは keep-warm でほぼ消え、
残ったのはたまにしか引かない名前の高いミスです。平常時の `wait` と `connect` の平均の差 6.0 ms はこのぶんで、
`dns_slow` の anomaly がミス 1 回で立つようになったのも同じ理由です (`TODO.md` の T17.1)。

**p95 の尾 (約 160〜168 ms) は相手までの距離のせいで、コードでは縮まない**。

- 平常時の p90 は `wait` 20.6 / `connect` 14.1 ms です。前の 1 枚 (p90 約 120 ms) より下がったのは、平常時に遠い相手へ行く本数が
  減ったため (100〜250 ms の区間 10.7% → 4.8%) で、プロキシの差ではありません。`run_delay_us` (走れずに待った時間) は conn 役で
  5.1 時間に合計 39 ms、CPU の絞りも 0 です。p50 は 8.1 → 7.8 ms で、1 往復 (1 RTT) のままです。

**直す前 (2026-09-10) との比較**は日付ごとの表に残してあります (`TODO.md` の**付録 A「デプロイ先の現在地の
古い版」**。2026-09-10 / 2026-09-12 / 2026-09-16 / 2026-09-18 / 2026-09-24 の 5 枚を 1 文字も変えずに移しました。2026-09-18 の 1 枚には
Phase 14 の名前解決 0.55 → 0.144 回/接続と、p50 6 ms に届かなかった理由、2026-09-24 の 1 枚には Phase 15 の `discord.com` 0.441 → 0.01 があります)。山場は 5 つで、
CONNECT 確立 (AAAA のあるホスト) **257 → 9 ms** (2026-09-12)、接続元から見た応答の p50 **327.8 → 8.0 ms** (同)、
RSS **215.9 → 21.1 MB** (先行確保 201.3 → 0、2026-09-16)、`GET /` **502 → 200** (2026-09-12)、
**原因の分からなかったエラー 12 件**が原因別と個票つきになったこと (2026-09-16) です。
...
(尾の 119 本すべてで SYN 再送 0・オリジン側の再送 0、カーネル RTT の分布も尾でない接続と同じ、
利用者と無関係な canary にも同じ尾が出ます)。ただし**原因はまだ測れていません** — 再起動も版の
入れ替えも生き延びているので、次の Phase は「まず測る」から始めます (`TODO.md` の T15.0 で計器を先に足し、そのあと T15.5)。
なお**名前解決はもう待ちの主役ではありません** (接続 1 本あたり 6.4 → 2.8 ms)。
...
- **DNS キャッシュ**: 名前解決の結果を 60 秒保持し、CONNECT ごとの解決をなくす。解決失敗時は古い結果で凌ぐ。
  **直近 60 秒に使われた名前は期限の 3/4 (45 秒) で裏で引き直す**ので、使い続けているホストは
  ...
  さらに **keep-warm**: **直近 3,600 秒 (`PROXY_DNS_WARM_SECS`) に 2 回以上使われた名前 (warm) は、
...
- **ヘルスチェック & メトリクス & 操作**:
  ...
  - **個票 (集計では読めない「誰が・いつ・なぜ」。T13.4)**: `/errors?n=100` で直近のエラー
    ...
    **いまの転送速度 `rate_bps`** (直近 5 秒のバイト/秒。下の `rate_bps_total` を参照)。`--lite` では空)。
    さらに**トンネル 1 本ごとの証拠** (T15.0 (4)。「回っているのか・待っているのか」を 1 行で見分けるための 5 つ):
    - **`tid`** — いま受け持っているスレッドの番号 (`gettid`)。`/profile` の `threads_top` の `tid` と同じ番号なので、
      **CPU を食っているスレッドと、そのスレッドが抱えている接続**を突き合わせられます。`0` (= 欄が出ない) は
      「預かり所にいる / 分からない」。預かり所から戻ると別のワーカーが受け持つので、そのたびに書き直します
    - **`half_closed`** (`client` / `origin`) と **`half_closed_secs`** — **先に EOF を出した側**と、そこから経った秒。
      閉じた理由 (`client_eof` / `server_eof`) と同じ出どころなので、閉じる前から「どちらが先に手を引いたか」が読めます
    - **`spins`** — 中継の輪が「`poll` に起こされたのに 1 バイトも進まなかった」回数の通算。
      **寿命に対して桁違いに大きければ空回り**です (T15.5 で直した形は、これが秒あたり数万で増え続けました)。
      直したあとは「`poll` が返した起床は必ず 1 回の読み書きにつながる」ので、ここは伸びません
    - **`revents`** (`{"client":"HUP|ERR","origin":""}`) — **前の周の** `poll` が返した旗 (`IN` / `OUT` / `ERR` / `HUP` / `NVAL`)。
      `HUP` や `ERR` が立ったまま `spins` が伸びていれば、誰も面倒を見ていない記述子があるということです。
      **預かり所から戻った直後の 1 回は旗を持ちません** (旗は中継の輪が持つので、戻るたびに空から始まります)
    - **`idle_secs`** — 1 バイトも動いていない秒 (5 秒ごとに history スレッドが数えます)。CONNECT の行だけで、
      `http` の行には出しません (累計バイトを置くのはトンネルだけのため)。
      **累計バイトを書くのは中継が待ちに入る回だけ**なので、`EAGAIN` に落ちずに進み続ける飽和したトンネルでは
      全速で流れていてもここが伸びます (`rate_bps` も同じ性質です)。`spins` と `rate_bps` と併せて読んでください

    **既定のままの欄は出しません** (`tid` 0 / `spins` 0 / 旗なし / 半閉じなし / `idle_secs` 0)。読む側は「**無ければ既定値**」と
    読んでください。`/recent` の `ms` が 0 の段を出さないのと同じ作法で、理由は応答の大きさです — この口は
    「**256 KiB に 1,000 本入る**」ことを守っており、1 行あたりの余りは 5 B しかありません。
    ただし**その 1,000 本は「証拠つきの行が少数である 1,000 本」です**: 実測で既定の 1 行は 255 B、`tid` と旗が出る行は 301 B
    (全部がそれなら 860 本)、5 つとも出る行は 442 B (同 588 本) で、そこを越えると `truncated` が `true` になり
    `shown` 本までしか出ません (ページ送り `offset=` が入るまでの既知の穴です)。

    このほか、`/dns?sort=age|host|misses&limit=300` で名前解決の表の中身 (ホスト・アドレス・解決からの秒・残り TTL・
  ...
  - **閉じた接続の個票 (T14.4)**: `/recent?n=200&offset=0&since=<epoch>&client=<ip>&sort=time|slow|bytes` (既定 200、最大 2,000)。
    ...
    **空回りの回数** (`spins`。中継の輪が「起こされたのに 1 バイトも進まなかった」回数の通算。寿命に対して桁違いに
    大きければ空回りです。T15.0 (4)) と **半閉じ** (`half_closed` = `client` / `origin` / `null`、
    `half_closed_ms` = 半閉じから反対側が閉じる (= この接続が終わる) までの ms。半閉じしていなければどちらも `null`。
    `/connections` の `half_closed` と同じ出どころなので、閉じる前に見た行とそのまま突き合わせられます)。
    **`/connections` と違って、この 3 つは 0 でも `null` でも必ず出します** (件数で切る口なので、
    「欄が無い」= 古い版と区別させるため)。
    ...
    **遅いだけの相手** (`client_eof` だが `stall_ms.client` が大きい) が区別できます。**書くのは接続の終了で 1 回だけ**で、要求ごとにも中継のバイトごとにも
    何も書きません。2,000 件の環状 (ありふれた 1 件 414 B。T15.0 (4) の `spins` と半閉じ 2 つで 363 B から +51 B)。`?since=` は「開いた時刻がこれ以降」、`?client=` は接続元の完全一致、
    ...
    **`ms.connect` と `ms.client_read` の読み方 (2026-09-18 に直しました。T15.0 (1))**: 接続の段は「確立までの全体 − 名前解決」
    ...
    (デプロイ先の実測: ミスした 54 本のうち 40 本が 0 ms。そのカーネル RTT の中央値は 11.4 ms)、同じぶんが `client_read` に
    二重に乗っていました。いまは**自分の窓の内側で払ったぶんだけ**を引き、`client_read` からは除きます。
    **見出しの確立 p50 (`/status` の `recent_quantiles.connect`、`/history` の `connect`、`/slo`) は 1 ms も変わりません**
    (時計そのものは動かしていないため)。なお **forward (http) で既にあるオリジン接続を使い回した要求**だけは、入口の判定が払った
    名前解決がどの段にも出ません (`dns` も `connect` も 0 のまま、`client_read` に乗ったまま)。直っているのは
    **新しくオリジンへ繋いだ要求**です
  ...
  - **確立時の SYN の再送 (T14.46)**: Linux では**オリジンへ繋がった直後に 1 回だけ**
    `getsockopt(SOL_TCP, TCP_INFO)` を読み、`tcpi_total_retrans` を `/recent` の 1 件に
    `syn_retrans` として残します。確立した直後は**まだデータを 1 バイトも送っていない**ので、
    この値は **SYN (3-way handshake) の再送回数そのもの**です。
    - **読み方**: Linux の既定 (`tcp_syn_retries`) では最初の SYN の 1 秒後・3 秒後・7 秒後…と
      送り直すので、**`syn_retrans` が 1 なら確立に約 1 秒、2 なら約 3 秒**よけいにかかっています。
      `ms.connect` が 1,000 ms 台・3,000 ms 台に飛んでいる個票はここが 1 以上のはずで、
      「相手が遅い」のではなく **SYN が途中で落ちた** (相手の受け入れ待ち行列が溢れた・
      経路で落ちた) ことが分かります
    - `kernel.tcp.syn_retrans` (T14.12) との違い: あちらは `/proc/net/netstat` の値で
      **ネットワーク名前空間まるごとの累計**なので、同じ機械の他のプロセスのぶんも入ります。
      こちらは**接続 1 本ごと**なので、「どの宛先へのどの接続が再送したか」まで分かります
      (溢れているのがこちらの待ち受けか相手の待ち受けかは、この 2 つを突き合わせて切り分けます)
    - 合計は `/status` の末尾の **`syn_retrans_total`**、ホスト別は `/hosts` (と `/status` の
      `hosts[]`) の **`syn_retrans`** です (**`clients[]` には出しません** — 接続元別には
      「繋ぎに行く先」が無いため)。**どちらもメモリだけ**なので再起動で 0 に戻ります
    - 費用は **CONNECT 1 本につき `getsockopt` 1 回**だけです。forward は
      **プールが接続を張るときだけ**通るので、使い回した要求では 1 度も読みません
      (要求ごとの増加は 0)。Linux 以外では常に 0 です
  - **中継の詰まりの向き (T14.42)**: 転送が遅いとき、遅いのは「クライアントの回線 (下り)」か
    「オリジン」か「利用者の上り」か。トンネルの中継は `splice` が `EAGAIN` で止まると
    `poll` で**書けるのを待つ**ので、その待ち時間を**書けなかった側ごと**に足せば切り分けられます。
    `/recent` の 1 件の `stall_ms` が**そのトンネル 1 本の合計 (ms)** です:
    - **`stall_ms.client` が大きい** = プロキシが**クライアントへ書けなかった** =
      利用者の下り回線か端末が遅い (読むのが追いつかない)。プロキシもオリジンも遅くありません
    - **`stall_ms.origin` が大きい** = プロキシが**オリジンへ書けなかった** =
      オリジンが遅いか、利用者の上りが速すぎてオリジンが飲みきれない
    - **両方 0 か小さい** = どちらの向きも詰まっていない (遅いとすれば確立か名前解決で、
      `ms.dns` / `ms.connect` を見ます)

    その窓の合計は `/history` の `transfer` の末尾 2 列 (`stall_client_ms_sum` /
    `stall_origin_ms_sum`) です。**時計を読むのは `poll` で書けるのを待ちに入る回だけ**で、
    64 KiB ごとでも `splice` ごとでもありません。詰まらない中継 (書けば必ず入る相手) では
    `POLLOUT` を立てる枝に 1 度も入らないので、**時計を 1 回も読みません**。
    `--lite` でも同じ数え方ですが、`--lite` は個票の枠を作らないので**出す場所がありません**。
    Linux 以外 (`io::copy` を 2 スレッドで回す従来版) は待ちの向きが分からないので常に 0 です
  ...
  - **記録を一括で止める / 接続元を復元できない形にする (`PROXY_RECORDS`。T14.41)**:
    ...
    **接続元 IP をハッシュに置き換える**かを選べます。
    ...
    - **`off`**: `/recent` `/errors` `/connections` `/clients` `/log` `/events` `/trace` `/bursts`
      **と `/readers` (内部エンドポイントを引いた接続元。T14.53) のどれにも 1 件も書きません**
      (どれも `"count": 0`、`/status` の `readers` も `[]`)。個票のファイル
      (`$HOME/.rust-http-proxy.recent`) にも 1 バイトも書かず、読み戻しもしません
      (`"persisted": false`。ファイル自体は消さないので `on` に戻して再起動すれば前の個票が
      また読めます)。**統計ファイル (`.rrd`) の接続元別の表は、次の書き出し (5 秒ごと) で
      空になります** — 接続元を 1 行も持たなくなるので、前の起動が残した生の IP も
      ディスクから消えます (ホスト別の表と履歴はそのままです)。
    ...
    - **`hashed`**: 個票は残しますが、**接続元 IP を 16 桁の 16 進**に置き換えます
      (例: `127.0.0.1` → `cb3c874474114702`)。**起動ごとの乱数 (塩) を混ぜた FNV-1a 64 ビット**
      なので、**同じ起動の中では同じ接続元がいつも同じ値**になり (`/recent` `/errors`
      `/connections` `/clients` `/trace` `/readers` `/bursts` の写真、`/recent?client=` の
      絞り込みで突き合わせられます)、**プロセスを再起動すると値が変わります** (= 値から IP を
      引き当てる表を作り置きできません)。`/clients` と `/readers` の鍵も同じ値です。
      前の起動が `on` のときに
  ...
  - **カーネルと cgroup の窓 (バーストのときカーネル側で何が起きていたか。T14.12)**:
    ...
    - **cgroup の CPU の絞り** (`cpu_nr_periods` / `cpu_nr_throttled` / `cpu_throttled_usec` と
      ...
      **`cgroup_cpu` と `since_start` の末尾の `usage_usec` / `user_usec` / `system_usec`** (T16.0) は同じ `cpu.stat` の
      CPU の使用量 (累計 us / 起動からの増分) で、**ユーザー空間とカーネル側**に分かれます (コンテナ全体。
      プロセスだけなら `/profile` の `cpu_user_us`)。行の無い `cpu.stat` では 3 つとも `null`。`/history` の `kernel` の
      末尾には同じものの増分が `cpu_user_usec` / `cpu_system_usec` で入ります (1 分へは足し合わせ)
    ...
    **読めない源は `null`** (Linux 以外、`/proc/net` の無いコンテナ、cgroup v1、PSI 無しのカーネル)。
    ...
    広がったので、入れるなら 26 列は収まります。移すかどうかは項目ごとに決めます)。
  ...
  - **RSS の内訳** (`/status` の `memory`。T14.21): 256 MiB のコンテナで「RSS が何でできているか」
    ...
    **読むのは `/status` に来たときだけ**で、要求の経路には 1 命令も増えません
    ...
    - `rings` は記録のリングが**満杯になったときの見積もり** (バイト): `recent` (2,000 件)、
      ...
      T15.0 (10) で標本 1 本が 504 → **664 B** になって **5.84 MB**、T16.0 で **688 B** / **6.00 MB**)、
      ...
      `quantiles` (3 系統 × 1,024 本 × 8 B = 24 KiB。T14.31、T15.0 (2) で `wait` を足して 16 → 24 KiB)、
      `profile` (段階とスレッドの窓 = 5 秒 × 720 + 60 秒 × 1,440 の標本。`.rrd` には書きません。
      T14.3。**T14.21 はこれを落としていた**ので `total` が RSS の説明に足りていませんでした。
      T15.0 (13) で足して 6.77 MB、T16.0 で 6.95 MB — `/profile` の標本 1 本は 3,216 B で、`history` より大きい口です) と、
      その合計 `total` (この機械では **14.3 MiB** = 15,022,620 B。T16.0 の前は 14.0 MiB)
    - `rings_used` は `rings` と**同じ鍵**で、**いま埋まっているぶんの実バイト** (T15.0 (13))。
      1 件あたりの見積もりは `rings` と同じものを使うので、必ず `rings_used.<名前> ≤ rings.<名前>` です。
      `rings` は 0 時間でも 36.6 時間でも 1 バイトも変わりませんが、こちらは溜まるにつれて増えます
  ...
  - **直近の窓の正確な分位点** (`/status` の `recent_quantiles`。T14.31):
    `{"connect":{"n":1024,"p50":0.712,"p90":1.204,"p99":3.410,"max":9.876,"window_secs":137},"forward":{…},"wait":{…}}`。
    ...
    **正確な値**なので、「p50 が 8.3 か 6.0 か」を 1 ms 単位で読めます。
    ...
    - **`wait` は利用者が待つ時間** (`queue + client_read + dns + connect` の和。**CONNECT だけ**。T15.0 (2))。
      ...
      基準線はこの `wait` を見てください**。まだ外にあるのは 2 つだけです: accept から要求行が届くまでと、
      `200 Connection Established` を書いた後のトンネル越しの TLS 握手 (`first_relay`)。
      形は `connect` / `forward` と同じ 6 つの欄で、**同じ 1 本の接続を数えるので `n` は `connect` と一致し、
      各分位点は `connect` の同じ分位点以上**になります。`--lite` では書きません
      (`queue` と `client_read` が 0 なので「4 段の和」を名乗れないため)。
      **`connect` の系列は 1 バイトも変えていない**ので、Phase 12〜14 の p50 と比べる値はそのままです。
  ...
  - `/history?res=5|60|3600[&n=720]` で **6 時間 / 1 日 / 30 日**の履歴。
    ...
    **`n=` は新しい方から何本返すかで、既定は今までどおり 720 本** (1 時間ぶん。応答の大きさを変えないため)、
    ...
    既定で全部返るので `n=` を書く意味はありません。増えたのは**メモリだけ** (標本 1 本 688 B (T16.0。T15.0 では 664 B) × 4,320 = **2.83 MiB**。
  ...
  - **`keys` の隣に `key_kinds`** が並びます (T15.0 (10))。`keys` と**同じ長さ・同じ並び**で、
    ...
    意図したもの (T12.4 (3)) なので、綴りは変えずに読み方を隣に出しています:
    - `time` = 窓の先頭の時刻 (epoch 秒)
    - `cumulative` = **起動からの通算**。レートが欲しければ隣の標本との差を取ってください
      (粗い解像度へは窓の最後の値で畳みます。再起動をまたぐと段差が 1 つ出ます)
    - `delta` = **その区間に起きたぶん**だけ (粗い解像度へは足し合わせ)
    - `gauge` = その瞬間の値 (粗い解像度へは平均。山は消えます)。
      ただし**上限の列 (`max_fds`) だけは窓の最後の値**で畳みます (`RLIMIT_NOFILE` は
      ほぼ定数なので、平均にすると上限が変わった瞬間だけ中途半端な数になります)
    - `peak` = その区間の最大 (粗い解像度へも最大)
    - `buckets` = **入れ子の配列** (要素ごとに足して畳みます。段の境目は `bounds_ms`、
      `errors_by_cause` の名前は `causes`)
  - 末尾に足した列 (T15.0。**既存の列は 1 つも消していません・意味も変えていません**):
    - **T15.0 より前に状態ファイル (`.rrd`) へ書かれた標本は、この 8 列が 0 で読み戻ります**
      (版を上げずにレコードの余白へ足したので、古いレコードはそこがゼロ埋めです)。
      再デプロイ直後の `/history` には、その 0 が `?res=5` で最大 1 時間・`?res=60` で最大 1 日・
      **`?res=3600` で最大 30 日 (720 本)** ぶん並びます。`t` が最後の再起動より前の標本では、
      **この 8 列を読まないでください** (下の `active_peak >= active_max` もそこでは成り立ちません)。
      再起動の時刻は `/status` の `uptime_secs` から出せます
    - **`waits` / `wait_ms_sum` / `wait_ms_max` / `wait_buckets`** = その区間の
      **利用者が待つ時間** (`queue + client_read + dns + connect` の ms。CONNECT だけ)。
      `connects` の 4 列と同じ形・同じ読み方 (区間) です。`connect_*` は `open()` の入口
      (要求行を読んだ後) からしか測らないので、accept してワーカーが動き出すまでの待ちも
      名前解決も入っていません。**「利用者が何 ms 待ったか」はこちらを読んでください**。
      `--lite` では `wait` の窓に 1 本も書きません (段の内訳を取らないので、4 段がそろう
      分岐に入りません。件数 0 のまま)
    - **`dns_warm`** = その標本の瞬間の **warm な名前の件数** (`/status` の `dns.warm` と同じ数。
      `gauge` なので粗い解像度へは平均で畳みます)。`/config` の `dns_warm`
      (`PROXY_DNS_WARM_SECS`) は warm に保つ窓の**秒数**で意味が違うので取り違えないでください。
      裏の引き直しの速さは **引き直し回数/時 ÷ その時間の平均 `dns_warm`** で 1 名前あたりになります
      (通算の平均で割ると実勢を 35〜60% 過小に見せます)。**引き直し回数は `/history` の列にはありません** —
      `/status` の `dns.refreshes` (起動からの通算) を 2 回引いて差を取ってください
  - 末尾に足した列 (T16.0): **`dns_warm_max`** (`peak`) = その区間の warm な名前の件数の**最大**
    (`dns_warm` は平均なので、枠 32 件がバーストで埋まったかは 60 秒と 1 時間の行からはこちらで読みます)、
    **`dns_warm_sum`** と **`gauge_n`** (どちらも `delta`) = その行に畳んだ 5 秒の標本の `dns_warm` の合計と標本の数
    (5 秒の行は `dns_warm` と同じ値と 1)。粗い解像度の**ゲージ 9 つ** (`active` / `mem_used` / `mem_limit` /
    `disk_used` / `disk_limit` / `rss` / `threads` / `fds` / `dns_warm`) の平均は T16.0 から**標本の数で重みを付けて
    四捨五入**します (前は行の数で割って切り捨てていたので、5 秒 → 60 秒 → 1 時間の 2 段で下に偏っていました)。
    `dns_warm` は合計から出すので真の平均 ±0.5、ほかの 8 つは 60 秒の丸めを重ねるので 1 時間の行で最悪 ±1.0 です。
    **T16.0 より前に書かれた標本はこの 3 列が 0** で読み戻り、畳むときは 1 本ぶん (`gauge_n` = 1) として扱います
    - **`requests_delta` / `bytes_delta`** = その区間に増えた要求数とバイト数。
      隣の `requests` / `bytes` は**通算のまま**残してあります (ダッシュボードと道具が読んでいます)。
      差は**標本を作るときに**取っているので、再起動の段差や欠けた標本があっても狂いません
    - **`active_peak`** = その区間の同時接続の**真の山**。`active_max` は 5 秒ごとの瞬間値の
      最大なので 5 秒より短い山を取りこぼしますが、こちらは接続を数え上げるたびに見ています
      (**T15.0 以降に撮った標本では** `active_peak >= active_max`。それより前の標本は
      上のとおり `active_peak` が 0 で読み戻るので、この不等式は成り立ちません)
    ...
    **`stall_client_ms_sum` / `stall_origin_ms_sum`** (中継が**書けるのを待った** ms の合計。T14.42。
    **末尾に足した 2 列**で、既存の並びは 1 つも変えていません。`client` は「クライアントへ書けなかった」=
    利用者の下り回線か端末、`origin` は「オリジンへ書けなかった」= オリジンか利用者の上り。
    `tunnels` で割れば 1 本あたりの平均。1 本ごとの値は `/recent` の `stall_ms`。
    **1 KiB の足切りは掛けず、終わったトンネル全部**が対象)。
  ...
  - **`/profile?res=5|60`** で「**1 要求の時間がどの段階に消えたか**」「**CPU がどの役割のスレッドで何をして使われたか**」
    ...
    - **`user_us` / `cpu_user_us` (ユーザー空間の CPU。T16.0)** — `keys` の末尾の 2 列。`user_us` は `roles` と同じ長さの配列で
      役割ごとの、`cpu_user_us` はプロセス全体の、その窓の CPU のうち**ユーザー空間** (utime の増分 us) です。
      **カーネル側は `threads` の `cpu_us − user_us` / `cpu_us − cpu_user_us`** で出します (sys は持ちません)。
      中継の CPU は大半がカーネル (`splice` / `poll`) なので、ビルドの設定のようにユーザー空間にしか効かない変更は
      合計では差が埋もれます。`recent` と `?summary=1` の各段の末尾にも `cpu_user_us` が付きます
    ...
    - 記録はプロセスのメモリだけ (約 6.9 MB = 1 標本 3,216 B × (720 + 1,440)。状態ファイルには書きません)。応答は 256 KiB 以下で、
      入り切らないときは**新しい標本を残して**古い方から落とし `"truncated": true` を出します。**`--lite` では `{"profile":"off"}`**
    - **`?n=` と `?offset=` (T15.0 (11))** — 応答の 256 KiB に対して標本 1 本の JSON の行が太いので、
      デプロイ先の実測 (T15.0 の行の形) で `?res=5` は 720 標本のうち **456 本**、`?res=60` は 1,440 標本のうち **362 本**しか
      1 枚に入りません (T16.0 で 1 行が 20〜90 B 太ったので少し減るはずですが、まだ測り直していません。
      上の 3,216 B はメモリの見積もりで、JSON の行の長さとは別の数です)。`n=` が 1 枚に返す標本の数
...
> このリポジトリは **約 90 MB のメモリでリリースビルドが通ります** (2026-09-26、手元の aarch64・rustc 1.98.1 で実測。
> 90 MB は測った 4 回とも通り、85 MB 以下は 2 回とも落ちます。
> CI が毎回確かめているのは **120 MB の関門を通るかどうかだけ**で、CI で通る最小は測っていません)。
> LTO を既定で切っているのと、全クレートを `codegen-units = 4` にしてあるのもこのためです (下の「プロファイルの設定」)。
> `rustc` はクレート単位で全部を一度に抱えるため、いちばん大きいクレート (`proxy-endpoints`) の 1 プロセスで
> **RssAnon 82.5 MB** 使い (通る最小はここに 7〜20 MB 足した値になります。`/usr/bin/time -v` の最大 RSS は
...
> (8 コアで 32.3 秒 → 14.9 秒)。
>
> 手元で確かめるなら `cargo clean && scripts/build-memory.sh 120` (通る最小を探すなら
> `scripts/build-memory.sh --find 80 85 90 95 100 110 120`)。このスクリプトは **実際にその上限の
...
リリースビルド → 短いベンチ、の順です。**120 MB の cgroup でビルドが通るか** (`scripts/build-memory.sh 120`) は
...
3. **ビルドが 120 MB に収まらない**: 静的リンクにしてもビルドが軽くなるわけではありません
...
| `opt-level` | `3` | `"s"` |
| `lto` | `false` | `true` (fat) |
| `codegen-units` / `strip` | `4` (全クレート) / あり | `1` / あり |
| ビルドが通る最小のメモリ | **約 90 MB** (手元。90 は 4 回とも通り、85 以下は毎回落ちる。2026-09-26。CI では未測定) | 350 MB 以上 |
| バイナリ | 2,760,504 B | **1,842,976 B** (852 KB 小さい) |
| CPU/要求 (forward 8 並列) | 41.4 us | **39.6 us** |

(バイナリの大きさと通る最小のメモリは 2026-09-26 (T17.11 = T15.12 段 8) の実測です。2026-09-24 (段 7) の時点は
`release` 2,694,968 B・通る最小 105〜110 MB、2026-09-19 (段 5・段 6') の時点は
`release` 2,563,896 B・通る最小 120〜125 MB、2026-09-08 の時点は `release` 1,381,872 B / `dist` 1,054,184 B (327 KB 小さい) でした。
CPU/要求 の 2 つは 2026-09-08 の値で、段 5 / 段 7 では測り直していません。速さの測り方は上の「性能」と同じです)

**LTO を既定で切っている理由**: `cargo build --release` を **メモリ 120 MB のコンテナで通す** ためです。
...
`fat` は効きますが 120 MB の約 3 倍のメモリが要ります。`thin` は 170 MB 払ってもバイナリが
8 バイトしか変わらず、速さもぶれの中です。配布バイナリだけ `dist` (fat LTO) で作ります。

**`codegen-units` は全クレート `4`** (2026-09-24、T15.12 段 7): ビルドの山はコード生成 (LLVM) の段で、
`codegen-units = 1` はクレート全体の LLVM IR を 1 つのモジュールとして一度に抱えます。
全クレートに当てて測った実測 (手元 aarch64、`jobs = 1`、RssAnon の最大) は
`proxy-metrics-watch` 121.7 → 92.5 MB (`= 4`) → 84.3 MB (`= 16`)、クリーンビルド 95 → 106 → 133 秒で、
**4 より上げても減りません** (コード生成の前の段 = 型検査とメタデータの床が約 80 MB)。
全クレート `4` のいまの上位は `proxy-endpoints` 82.5 > `proxy-server` 81.5 > `proxy-metrics-recent` 76.9 >
`proxy-http` 74.7 MB です (2026-09-26、T17.11 で `proxy-metrics-watch` 95.4 MB と `proxy-metrics-window` 91.1 MB を
2 つずつに割った後。段 7 の前は `proxy-metrics-window` 107.8 MB がいちばん上)。
`codegen-units` を増やすとクレートの中でのインライン化の範囲が狭まりますが、手元もデプロイ先も共有の機械で
CPU/要求 が数 % ぶれるので手元の A/B は取らず、**デプロイ先で「桁で悪くなっていないか」だけ**を見ます。
悪くなっていたら `Cargo.toml` の `[profile.release]` の `codegen-units` を `1` に戻す 1 行で戻ります。
`--lite` のシステムコール 5.022 → 5.023 回/要求、確保 10.006 → 10.006 回/要求 で、要求の経路の費用は動いていません。
2026-09-19 (段 5) から 2026-09-24 (段 7) までは、要求の経路に乗らない 9 クレートだけを
`[profile.release.package.<名前>]` で `4` にしていました。
`dist` は `codegen-units = 1` を明示しています (release の `4` を継ぐと fat LTO でも `.text` が 16.9 KB 太るため)。

**最適化レベル**: `release` は `opt-level = 3`、`dist` は `"s"` です (`dist` はサイズを優先)。
fat LTO と合わせた `dist` の実測 (2026-09-08) は `release` に対して forward 8 並列 **+1.9%** (CPU/要求 41.4 → 39.6 us で -4.4%)、
キャッシュ HIT +0.7%、64 並列 -1.2%、CONNECT 確立 -8% (この経路のぶれ ±25% の中) で、**CPU 以外はほぼぶれの中**です。
それでも `dist` は 721 KB 小さい (2026-09-19 の実測) ので、配布は `dist` のままにしています。
...
**重い口は同時に 1 本だけ組みます** (つまみはありません。いつもこの動作です)。`/snapshot` は 17 本ぶんの口を
1 つにまとめた**最大 4 MiB** の JSON で、認証が無いので誰でも好きなだけ叩けます。1 秒に 10 回引かれると、それだけで
CPU と統計の鍵の時間を食います。そこで**大きい応答を組む口は同時に 1 本**とし、組んでいる最中に来た 2 本目からは
**`503 Service Unavailable` + `Retry-After: 1`** (本文は `{"schema":1,"error":"busy"}`) で断ります。重い口はこの 6 つです:
...
| `/history` | `n` が **720 より大きい**とき (`/history?res=5&n=4320` は 1.9 MB) |

既定の `limit` / `n` のまま引くぶんには重い口になりません。`/status` `/healthz` `/metrics` のような軽い口は
**この判定にも旗にも触りません** (監視が 5 秒ごとに叩く口を断ったら、断ったこと自体が事故になるため)。
断った累計は `/status` の末尾の **`heavy_rejected`**。**走査に対する最小限の保護で、認証ではありません** —
順に引けば誰でも全部 200 で取れますし、`Retry-After: 1` のとおり 1 秒後に引き直せばまず通ります。
旗は**組み終わったところ**で戻すので、読むのが遅い相手 1 人が全員に 503 を返させることはありません。
費用は重い口に来たときの原子 1 回だけで、プロキシとしての転送 (forward / CONNECT) には 1 命令も足していません。
...
**`dashboard.html` の大きさの上限は 80 KiB** です (T15.0 (14) で下の 5 枚を足して 81,673 B。
`node scripts/check-dashboard.js` が見張ります。外部ライブラリを読み込まない 1 ページという方針を
守るための歯止めなので、超えたら中身を削るか上限を上げるかを先に決めてください — **余白は 247 B しかありません**)。
...
**`syn_retrans`** (そのホストへ繋ぐときに **SYN を送り直した回数の合計**。T14.46。確立の直後に
`TCP_INFO` を 1 回読んだ値で、`retrans` (接続の終わりに読んだ通算) と違って**確立の失敗だけ**を数えます。
1 回で確立が約 1 秒、2 回で約 3 秒よけいにかかるので、`avg_ms` が飛んでいるホストはここを見てください。
**メモリだけ**なので再起動で 0 に戻ります。合計は `/status` の末尾の `syn_retrans_total`)。
...
さらに **`misses_by_kind`** (ミスの内訳。`cold` = 表にまだ答えが無い / `expired` = 答えは持っていたが
TTL を過ぎた / `warm_stale` = **warm なのにミスした** = 裏の引き直しが間に合っていない /
`negative` = 覚えている失敗の期限が切れたので引き直した。**4 つの和は必ず `misses` と一致します**) と、
裏の引き直しの様子 **`refresh_failures`** (引き直しが失敗した回数) / **`refresh_ms_sum`** /
**`refresh_ms_max`** (引き直しの `getaddrinfo` にかかった時間の合計と最大 ms) /
**`refresh_late`** (予定の時刻から **5 秒以上**遅れて始まった引き直しの回数。引き直しは
`dns-refresh` スレッド 1 本が順にやるので、1 回 約 2 秒かかる名前が続くと後ろが詰まります。
要求の経路が頼んだ先回りは予定の時刻を持たないので数えません) が出ます。
末尾の **`warm_evicted`** (T16.0) は warm の枠 (同時に 32 件) が満杯で、新しく warm になる名前のために
**最後の使用がいちばん古い名前を外した**回数の起動からの通算です (`/history` の `dns_warm` は 5 秒ごとの瞬間値なので、
枠が埋まって押し出しが起きていたかはこれで読みます)。**要求の経路の例外**: 数えるのはその名前が窓の中で 2 回目に
使われ、かつ枠が満杯のときの枝だけで、既に握っている鍵の内側の原子の加算 1 つです (システムコールも確保も増えません)。
**読み方**: `expired` が主なら窓 (`PROXY_DNS_WARM_SECS`) の外で期限が切れている =
窓を延ばすと効く、`warm_stale` が出ていれば引き直しが間に合っていない = 窓ではなく
`refresh_failures` / `refresh_late` を先に見る、という順で読みます (どちらに手を入れるかは
この内訳でしか決まりません)。
...
数字の出どころと期待値は `TODO.md` の「再デプロイの手引き」にあります。
...
入りません。T15.0 の行の形での実測で、T16.0 の欄のぶん少し減ります)。**既定では追いません** — 続きはどれも重い口で、重い口は同時 1 本 (上の「重い口は同時に 1 本だけ」) なので
...
- **判定表の「平常時」は本数で切っています** (1 標本 300 本未満)。同時接続の山ではなく本数で切るので、
  同時接続 32 の山が来た 1 時間を**残し**、同時接続 6〜8 で本数だけ多い時間を**捨てる**ことがあります。
  実測では切り方を 9 通り変えると名前解決のミス率が **0.094〜0.171** に振れました。決まりどおりの 0.144 は
  いちばん悪い値ではなく、本数の閾を 200 本 / 500 本にすると **0.156 / 0.171** で基準 0.15 を越えます。
  切らずに全時間で読むと 0.147 (基準に対する余裕 2%、0.144 なら 4%)、同時接続で切ると **0.094** まで下がります。
  **判定がぎりぎりのときは切り方を変えて幅も見てください** (内訳は `TODO.md` §2 の「物差しの但し書き」)。
- **雪像の `dropped` が `[]` でも欠けていないとは限りません**。`dropped` は「部ごと落とした」の意味で、
  部の中の打ち切り (1 部 256 KiB) は**各部の `truncated` / `count` / `shown`** に出ます。2026-09-18 の雪像は
