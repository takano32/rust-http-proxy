...
| `/profile` の段階の内訳 (CONNECT 1 本の時間がどこに消えたか) | (無かった) | 6.0 時間・433 本の平均で **queue 4.93 / client_read 1.12 / dns 1.13 / connect 15.98 / first_relay 20.73 / relay 377.6 / park 71,308 ms**。`queue` は二峰性で **9.2% が 25〜76 ms**・100 ms 超は 0 本。forward 97 本は origin 26.4 / ttfb 50.0 ms (うち 76 本はキャッシュ HIT で <1 ms) | `/profile?res=60` (1,440 標本のうち 362 しか返らないので 6.0 時間ぶん) |
...
| RSS | 21.1 MB | **41.8 MB** (十進) | `$.status.memory.rss` |
...
入れ替えも生き延びているので、次の Phase は「まず測る」から始めます (`TASKS.md` の T15.5)。
...
- **判定表の「平常時」は本数で切っています** (1 標本 300 本未満)。同時接続の山ではなく本数で切るので、
  ...
  実測では切り方を 9 通り変えると名前解決のミス率が 0.094〜0.147 に振れました (決まりどおりの 0.144 が
  そのうち最も悪い値)。**判定がぎりぎりのときは切り方を変えて幅も見てください**。
