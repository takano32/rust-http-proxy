...
(ビルドはメモリ 105〜110 MB 前後で通ります。下の「ビルド・テスト」を参照)。
...
> このリポジトリは **105 MB 前後のメモリでリリースビルドが通ります** (2026-09-24、手元の aarch64 で実測。
...
| ビルドが通る最小のメモリ | **105 MB 前後** (手元。105 は境目で回ごとに揺れる。CI では未測定) | 350 MB 以上 |
